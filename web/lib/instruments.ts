...
export const instruments = [
  { id: 'crosscheck', name: 'CrossCheck', category: 'Web QA',
    ...
    preview: 'The results shown come from an owned demo app.' },
  { id: 'surgeline', name: 'SurgeLine', category: 'Form automation',
    ...
    preview: 'The crash-recovery demo uses synthetic records and an owned form with server-side duplicate protection.' },
  { id: 'driftwatch', name: 'DriftWatch', category: 'Data monitoring',
    ...
    preview: 'The detection reading comes from deliberately planted failures in an owned test site.' },
  { id: 'duewatch', name: 'DueWatch', category: 'Expiry & follow-up',
    ...
    preview: 'The demo uses synthetic contracts and mock message delivery. Live sending is not demonstrated.' },
  { id: 'brandwall', name: 'BrandWall', category: 'Visual design QA',
    ...
    preview: 'The screenshots use generated brand assets on an owned test app, with no claimed brand affiliation.' },
] as const;

export type InstrumentId = typeof instruments[number]['id'];
export type ChapterState = { reveal: number; orbit: number; index: number; transition: number; outro: number };
